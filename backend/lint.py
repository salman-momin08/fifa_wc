...
- Bare exception handlers (except Exception: without re-raise)
...
def run_lint_check():
    """Run comprehensive lint checks across app and tests directories."""
    print(f"\n{BOLD}{CYAN}=== FIFA WC 2026 Code Quality & Lint Auditor ==={RESET}\n")
    ...
    for filepath in sorted(all_py_files):
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if node.returns is None and not node.name.startswith("test_") and node.name != "override_get_db":
                    warnings += 1

            # Check bare except Exception without raise
    ...
    if total_issues == 0:
        print(f"{GREEN}{BOLD}PASSED! All {files_scanned} files passed code quality checks with 0 errors.{RESET}\n")
        sys.exit(0)
    else:
        print(f"{YELLOW}Found {total_issues} suggestion(s) across scanned files.{RESET}\n")
        sys.exit(0)
